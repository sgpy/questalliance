...
@click.option('--backend_host_port', type=int, default=8080, help="Backend server port")
...
def deploy_relay_server(port, backend_host_name, backend_host_port, local):
    ...
    Default: Runs on 5000 and connects to backend http://localhost:8080
    """
...
@click.command()
@click.option('--port', type=int, default=8080, help="server port to run on")
@click.option('--local', type=bool, is_flag=False, help="Backend server port")
